...
class TransE:
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            # adding an early stop mechanism
            if count_stop > 2:
                break
            start = time.time()
            self.loss = 0
            ...
            if epoch % 10 == 0:
                with codecs.open(f"../res/entity_temp_{data_name}", "w") as f_e:
                    for e in self.entity.keys():
                        f_e.write(str(e) + "\t")
                        f_e.write(str(list(self.entity[e])))
                        f_e.write("\n")
                with codecs.open(f"../res/relation_temp_{data_name}", "w") as f_r:
                    for r in self.relation.keys():
                        f_r.write(str(r) + "\t")
                        f_r.write(str(list(self.relation[r])))
                        f_r.write("\n")
                with codecs.open(f"../res/result_temp_{data_name}", "a") as f_s:
                    f_s.write("epoch: %d\tloss: %s\n" % (epoch, self.loss))

        print("写入文件...")
        with codecs.open(f"../res/entity_50dim_{data_name}_batch{nbatches}", "w") as f1:
            for e in self.entity.keys():
                f1.write(str(e) + "\t")
                f1.write(str(list(self.entity[e])))
                f1.write("\n")

        with codecs.open(f"../res/relation_50dim_{data_name}_batch{nbatches}", "w") as f2:
            for r in self.relation.keys():
                f2.write(str(r) + "\t")
...
if __name__ == '__main__':
    ...
    data_name = 'yago3-10'
    file1 = f"../dataset/{data_name}/"
    entity_set, relation_set, triple_list = train_loader(file1)
    print("load file...")
    print("Complete load. entity : %d , relation : %d , triple : %d" % (
        len(entity_set), len(relation_set), len(triple_list)))
    transE = TransE(entity_set, relation_set, triple_list, data_name, embedding_dim=50, learning_rate=0.02, margin=1, L1=True)
